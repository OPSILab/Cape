...
export class LoginPopupComponent implements AfterViewInit, OnDestroy {
  ...
  completeLogin = (token: OidcJWTToken): void => {
    try {
      ...
      localStorage.setItem('serviceAccountId', tokenPayload.email);
  ...
  closeLoginPopup = (): void => {
    ...
    if (redirectAfterLogin) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
      window.opener.document.location.href = this.onlineServicesUrl + redirectAfterLogin + (queryString ? queryString : '');
      // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    } else window.opener.document.location.href = this.onlineServicesUrl;
