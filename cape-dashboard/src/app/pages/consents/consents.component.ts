import { Component, ElementRef, Input, OnInit, OnDestroy, ViewChild, TemplateRef, ChangeDetectorRef, ViewChildren, QueryList } from '@angular/core';
...
import { ConsentStatusEnum, ConsentStatusRecordPayload } from '../../model/consents/consentStatusRecordPayload';
...
export class ConsentsComponent implements OnInit, OnDestroy {
  ...
  private consentUpdateConflict: TemplateRef<any>;
  ...
  constructor(private route: ActivatedRoute, private service: ConsentsService, private linkedServicesService: LinkedServicesService, private router: Router,
    private translate: TranslateService, private configService: NgxConfigureService,
    private loginService: LoginService, private modalService: NbDialogService, private toastrService: NbToastrService,
    private errorDialogService: ErrorDialogService, private changeDetector: ChangeDetectorRef) {
    ...
    this.changeStatusButtons = [
      {
        id: 1,
        name: this.translate.instant('general.consents.activate'),
        color: 'success'
      },
      {
        id: 2,
        name: this.translate.instant('general.consents.disable'),
        color: 'warning'
      },
      {
        id: 3,
        name: this.translate.instant('general.consents.withdraw'),
        color: 'danger'
      }
    ];
  }
  ...
  async getConsents(consentId?: string, serviceId?: string, status?: ConsentStatusEnum, purposeCategory?: string, processingCategory?: ProcessingBasisProcessingCategories) {
    ...
    try {
      ...
      if (this.consents.length === 0)
        this.message = this.translate.instant('general.consents.no_consent_message');
      else
  ...
  isActiveConsent(consentIndex): boolean {

    return this.consents[consentIndex].sink.consentStatusList[this.consents[consentIndex].sink.consentStatusList.length - 1]
      .payload.consent_status === ConsentStatusEnum.Active;
  }
  ...
  onChangeStatusClick(consentIndex: number, buttonId: number): void {
    ...
    switch (true) {
      ...
      default: {
        newStatus = this.consents[consentIndex].sink.consentStatusList[this.consents[consentIndex].sink.consentStatusList.length - 1]
          .payload.consent_status;
        break;
      }
    }
    this.updateConsent(consentIndex, newStatus);
  }
  ...
  isActiveButton(consentIndex: number, buttonId: number): boolean {
    ...
    return this.changedDataMapping[consentIndex] === false &&
      this.changedShareWith[consentIndex] === false && (buttonId === 1 && status === ConsentStatusEnum.Disabled
        || buttonId === 2 && status === ConsentStatusEnum.Active
        || buttonId === 3 && (status === ConsentStatusEnum.Active || status === ConsentStatusEnum.Disabled));
  }


  async updateConsent(consentIndex: number, newStatus?: ConsentStatusEnum) {
    ...
    if (this.changedDataMapping[consentIndex])
      lastDataset.dataMappings = this.lastClickedDataMapping.get(consentIndex).reduce((result, concept) => {
    ...
    if (this.changedShareWith[consentIndex])
      lastUsageRules.shareWith = this.lastClickedShareWith.get(consentIndex).reduce((result, shareWith) => {
    ...
    try {
      const updatedStatus = await this.service.updateConsentStatus(consent.payload.common_part.slr_id, consent.payload.common_part.cr_id, newResourceSet, status, lastUsageRules);
      ...
      this.openDetails(consentIndex, false);
  ...
  async forceServiceLinkAndConsentActivation(consentIndex: number, serviceId: string, slrId: string) {
  ...
  async getConsentDataMapping(consentIndex): Promise<DataMapping[]> {
    ...
    const consentDataset: Dataset = consentStatusLength > 1 ? consent.sink.consentStatusList[consentStatusLength - 1].payload.consent_resource_set.datasets[0]
    ...
    const consentDataMappingMap: Map<string, DataMapping> = new Map(consentDataset.dataMappings.map(dataMapping => [dataMapping.conceptId, dataMapping]));
  ...
  changeDataMapping(newValue: boolean, consentIndex: number, conceptIndex: number): void {
    ...
    if (consentIndex === this.lastClickedIndex) {
      ...
      this.changedDataMapping[consentIndex] = this.lastClickedDataMapping.get(consentIndex).reduce((count, concept) => {

        if (concept.initial !== concept.current)
          count++;
        return count;
      }, 0) > 0;
    }
  ...
  async getShareWithByConsentIndex(consentIndex): Promise<ShareWith[]> {
    ...
    const consentDataset: Dataset = consentStatusLength > 1 ? consent.sink.consentStatusList[consentStatusLength - 1].payload.consent_resource_set.datasets[0]
      : consent.sink.payload.common_part.rs_description.resource_set.datasets[0];
    const consentShareWith: ShareWith[] = consentStatusLength > 1 ?
    ...
    const purposeId: string = consentDataset.purposeId;
    const sinkShareWith: ShareWith[] = (await this.service.getServiceProcessingBasis(consent.sink.payload.common_part.subject_id, purposeId)).shareWith;
  ...
  changeShareWith(newValue: boolean, consentIndex: number, shareWithIndex: number): void {
    ...
    if (consentIndex === this.lastClickedIndex) {
      ...
      this.changedShareWith[consentIndex] = this.lastClickedShareWith.get(consentIndex).reduce((count, shareWith) => {

        if (shareWith.initial !== shareWith.current)
          count++;
        return count;
      }, 0) > 0;
    }
  ...
  async openDetails(consentIndex: number, changeCollapse = true) {
    ...
    if (changeCollapse)
      this.isCollapsed[consentIndex] = !this.isCollapsed[consentIndex];
  }

  openConsentHistory(consentHistory, consentIndex) {
  ...
  openDatasetHistory(datasetHistory, consentIndex) {
  ...
  async onFilterSubmit() {
    ...
    await this.getConsents(undefined, this.filtersForm.get('service').value, this.filtersForm.get('status').value, this.filtersForm.get('purposeCategory').value, this.filtersForm.get('processingCategory').value);
  }
