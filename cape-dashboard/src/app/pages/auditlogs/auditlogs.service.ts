...
export class AuditLogsService {
  ...
  getAuditLog(): Promise<AuditLog> {
    ...
    return this.http
      .get<AuditLog>(`${this.auditLogUrl}/auditLogs/accounts/${this.accountId}`).toPromise();
  }
  ...
  getEventLogs(): Promise<EventLog[]> {
    ...
    return this.http
      .get<EventLog[]>(`${this.auditLogUrl}/eventLogs/accounts/${this.accountId}`).toPromise();
  }


  getFilteredEventLogs(legalBasisFilter: ProcessingBasisLegalBasis[], dateRangeFilter: DateRange, processingFilter: ProcessingBasisProcessingCategories[]): Promise<EventLog[]> {

    let queryParams = new HttpParams();
    if (legalBasisFilter)
      for (const legalBasis of legalBasisFilter)
        queryParams = queryParams.append('legalBasis', legalBasis);
    ...
    if (processingFilter)
      for (const processing of processingFilter)
        queryParams = queryParams.append('processingCategories', processing);
    ...
    return this.http
      .get<EventLog[]>(`${this.auditLogUrl}/eventLogs/accounts/${this.accountId}`, httpOptions).toPromise();
  }
