...
@Component({
  template: `
    <button
      ...
      size="small"
      status="primary"
      (click)="openRegisterService(registerService)"
    >
    ...
    <ng-template #registerService let-ref="dialogRef">
      <nb-card>
        ...
        <nb-card-footer class="d-flex justify-content-center">
          <button nbButton status="danger" size="small" (click)="onRegisterService(ref)">
  ...
  onRegisterService = async (dialogRef: NbDialogRef<unknown>) => {
    try {
      ...
      this.toastrService.primary(
        '',
        this.translate.instant('general.services.service_registered_message', { serviceName: this.value.serviceDescription.serviceName }),
