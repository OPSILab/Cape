...
export class EditorComponent implements OnInit {
  ...
  @ViewChild("confirmSaveDialog", { static: false })
  confirmSaveDialogTemplate: TemplateRef<any>;
  @ViewChild("confirmUpdateDialog", { static: false })
  confirmUpdateDialogTemplate: TemplateRef<any>;
  ...
  constructor(
    ...
    this.schemaDir =
      (this.config.serviceEditorUrl.includes("localhost")
        ? ""
        : this.config.serviceEditorUrl) + this.config.editorSchemaPath;
    this.loading = true;
  }
  ...
  ngOnInit() {
    this.sub = this.route.params.subscribe((params) => {
      ...
      if (params["serviceId"]) {
        this.service.getService(params["serviceId"]).subscribe((result) => {
          console.log(result);
      ...
      var watcherCallback = function (path) {
        ...
        console.log(
          "field with path: [" +
            path +
            "] changed to [" +
            JSON.stringify(this.getEditor(path).getValue()) +
            "]"
        );
        ...
        if (value !== '"undefined"' && value !== '""') {
          ...
          this.getEditor(
            path.substr(0, path.lastIndexOf(".") + 1) + "name"
          ).setValue(nameValue);
        }
      };
  ...
  importAsFile() {
    this.dialogService
      .open(DialogImportPromptComponent)
  ...
  private showToast(type: NbComponentStatus, title: string, body: string) {
    ...
    this.toastrService.show(body, `${titleContent}`);
  }
}
