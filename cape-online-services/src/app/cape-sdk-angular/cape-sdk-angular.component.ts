...
export class CapeSdkAngularComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private nbMenuService: NbMenuService, private capeService: CapeSdkAngularService,
    private toastrService: NbToastrService, private cdr: ChangeDetectorRef,
    private route: ActivatedRoute, private router: Router, private errorDialogService: ErrorDialogService,
  ...
  async ngOnInit() {
     ...
     */
    this.nbMenuService.onItemClick().pipe(takeUntil(this.unsubscribe))
      .pipe(
            ...
            case 'Link Service':
              this.capeService.serviceLinkStatus$.pipe(takeUntil(this.unsubscribe)).subscribe(async event => {
                ...
                if (event?.serviceId === this.serviceId)
                  ...
                  this.menuItems = [{ title: this.translateService.instant('general.services.disableServiceLinkLabel'), target: 'Disable Service Link' },
              ...
              await this.capeService.linkWithOperator(this.sdkUrl, this.locale, this.operatorId, this.serviceId, this.serviceName, this.userId, this.returnUrl, this.cdr);
              break;
    ...
    try {
      ...
      if (serviceDescription)
        this.isServiceRegistered = this.capeService.emitIsRegisteredValue(true);
    } catch (error) {
      if (error.status !== 404)
        this.errorDialogService.openErrorDialog(error);
      else {
     ...
     */
    try {
      this.serviceLinkRecord = await this.capeService.getServiceLinkRecordByUserIdAndServiceId(this.sdkUrl, this.userId, this.serviceId, this.operatorId);
    ...
    if (!this.serviceLinkStatus)
      this.menuItems.push({ title: this.translateService.instant('general.services.linkServiceLabel'), target: 'Link Service' });
    else if (this.serviceLinkStatus === SlStatusEnum.Removed)
      this.menuItems.push({ title: this.translateService.instant('general.services.enableServiceLinkLabel'), target: 'Enable Service Link' });
    else {

      /*
      * Service Linked. Check Consent Record
      */
      this.menuItems.push({ title: this.translateService.instant('general.services.disableServiceLinkLabel'), target: 'Disable Service Link' });

      try {
        // Get the first retrieved Consent Record (results are order by descending "iat" field)
        this.consentRecord = (await this.capeService.getConsentsByUserIdAndServiceIdAndPurposeId(this.sdkUrl, this.userId, this.serviceId, this.purposeId, this.operatorId, this.checkConsentAtOperator))[0];
      ...
      if (this.consentStatus === ConsentStatusEnum.Active)
        this.menuItems.push({ title: this.translateService.instant('general.consent.disableConsentLabel'), target: 'Disable Consent' },
  ...
  async startLinkingAfterServiceLogin() {
    ...
    try {
      ...
      const successMessage: string = this.translateService.instant('general.services.linkingSuccessfulMessage', { serviceId: this.serviceId, serviceName: this.serviceName });
  ...
  closeLinkedDialogAndReturnToOperator(message: string) {
    ...
    window.onunload = () => {
      window.opener.postMessage({
        result: 'SUCCESS',
        message: message, serviceId: this.serviceId, returnUrl: this.returnUrl
      }, this.returnUrl);
    };
    window.close();
  }
    ...
    try {
      this.dialogService.open(ConsentFormComponent,
        {
          ...
          context: {
            sdkUrl: this.sdkUrl,
            consentForm: await this.capeService.fetchConsentForm(this.sdkUrl, this.userId, this.serviceId, this.operatorId, this.purposeId, this.sourceServiceId, this.sourceDatasetId)
          }
        });
      ...
      this.capeService.consentRecordStatus$.subscribe(event => {
        if (event?.consentRecord) {
          ...
          this.menuItems = [
            { title: this.translateService.instant('general.goToDashboard') },
            { title: this.translateService.instant('general.services.disableServiceLinkLabel'), target: 'Disable Service Link' },
            { title: this.translateService.instant('general.consent.disableConsentLabel'), target: 'Disable Consent' },
            { title: this.translateService.instant('general.consent.withdrawConsentLabel'), target: 'Withdraw Consent' }
          ];
        }
      });
  ...
  async enableServiceLink(): Promise<void> {
    ...
    try {
      this.serviceLinkRecord.serviceLinkStatuses.push(await this.capeService.enableServiceLink(this.sdkUrl, this.serviceLinkRecord.payload.link_id, this.serviceLinkRecord.payload.surrogate_id, this.serviceId, this.serviceName));
      this.menuItems = [
        { title: this.translateService.instant('general.goToDashboard') },
        { title: this.translateService.instant('general.services.disableServiceLinkLabel'), target: 'Disable Service Link' }
      ];
  ...
  async disableServiceLink(): Promise<void> {
    ...
    try {
      this.serviceLinkRecord.serviceLinkStatuses.push(await this.capeService.disableServiceLink(this.sdkUrl, this.serviceLinkRecord.payload.link_id, this.serviceLinkRecord.payload.surrogate_id, this.serviceId, this.serviceName));
      this.menuItems = [
        { title: this.translateService.instant('general.goToDashboard') },
  ...
  async disableConsent(): Promise<void> {
    ...
    try {
      ...
      this.menuItems = [
        { title: this.translateService.instant('general.goToDashboard') },
        { title: this.translateService.instant('general.consent.enableConsentLabel'), target: 'Enable Consent' }];

      this.menuItems.push(this.serviceLinkStatus === SlStatusEnum.Active ?
        { title: this.translateService.instant('general.services.disableServiceLinkLabel'), target: 'Disable Service Link' }
        :
        { title: this.translateService.instant('general.services.enableServiceLinkLabel'), target: 'Enable Service Link' });
