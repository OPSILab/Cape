 ...
 * */
  ...
  constructor(private configService: NgxConfigureService, private translateService: TranslateService,
    private capeService: CapeSdkAngularService, private cdr: ChangeDetectorRef) {
    ...
    if (this.locale === 'it')
      this.sampleSex = this.sampleSex === 'Male' ? 'Maschio' : 'Femmina';
  }
  ...
  async ngOnInit() {
    ...
    this.capeService.consentRecordStatus$.pipe(takeUntil(this.unsubscribe)).subscribe(async event => {
      ...
      if (event?.serviceId === this.serviceId) {
        ...
        this.ageConsentActive = event.status.consent_resource_set.datasets.filter(dataset => dataset.dataMappings.filter(dm => dm.property === 'age').length > 0).length > 0
          && this.capeConsentStatus === ConsentStatusEnum.Active;

        this.sexConsentActive = event.status.consent_resource_set.datasets.filter(dataset => dataset.dataMappings.filter(dm => dm.property === 'sex').length > 0).length > 0
          && this.capeConsentStatus === ConsentStatusEnum.Active;
      }
    });
